...
use pyo3::prelude::*;
use pyo3::exceptions::{PyKeyError, PyValueError, PyIOError};
...
impl NativeStore {
    ...
    fn put_batch(&self, items: &Bound<'_, PyList>) -> PyResult<usize> {
        ...
        for item in items.iter() {
            ...
            if tuple.len() != 2 {
                return Err(PyValueError::new_err("Each item must be a (doc_id, doc) tuple"));
            }
    ...
    fn put_raw_batch(&self, items: &Bound<'_, PyList>) -> PyResult<usize> {
        ...
        for item in items.iter() {
            let tuple = item.downcast::<PyTuple>()
                .map_err(|_| PyValueError::new_err("Expected list of (doc_id, json_bytes) tuples"))?;
            if tuple.len() != 2 {
                return Err(PyValueError::new_err("Each item must be a (doc_id, json_bytes) tuple"));
            }
            let doc_id: String = tuple.get_item(0)?.extract()?;
            let json_bytes: Vec<u8> = tuple.get_item(1)?.extract()?;
            
            // Write raw bytes directly - no parsing!
            store.put_raw_line(&doc_id, &json_bytes)
                .map_err(|e| PyIOError::new_err(format!("Write failed: {}", e)))?;
            count += 1;
        }
    ...
    fn write_jsonl(&self, jsonl_blob: &[u8], doc_ids: Vec<String>) -> PyResult<usize> {
        ...
        store.write_jsonl_blob(jsonl_blob, &doc_ids)
            .map_err(|e| PyIOError::new_err(format!("Write failed: {}", e)))
    }
...
impl ScanIterator {
    ...
    fn __next__(&mut self, py: Python<'_>) -> Option<PyObject> {
        self.scanner.next().map(|v| json_to_py(py, &v).ok()).flatten()
    }
}
