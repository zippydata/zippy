...
use std::io::{BufWriter, Write, BufReader, BufRead, Read, Seek, SeekFrom};
...
impl FastStore {
    ...
    fn load_index_binary(path: &Path, index: &mut FxHashMap<String, IndexEntry>) -> Result<()> {
        ...
        for _ in 0..count {
            ...
            let offset = u64::from_le_bytes([entry_buf[0], entry_buf[1], entry_buf[2], entry_buf[3],
                                             entry_buf[4], entry_buf[5], entry_buf[6], entry_buf[7]]);
            let length = u32::from_le_bytes([entry_buf[8], entry_buf[9], entry_buf[10], entry_buf[11]]);
            
            index.insert(doc_id, IndexEntry { offset, length, _padding: 0 });
        }
    ...
    pub fn put_raw_line(&mut self, doc_id: impl Into<String>, line_bytes: &[u8]) -> Result<()> {
        ...
        // Update index
        self.index.insert(doc_id, IndexEntry {
    ...
    pub fn write_jsonl_blob(&mut self, jsonl_data: &[u8], doc_ids: &[String]) -> Result<usize> {
        let writer = self.writer.as_mut().ok_or_else(|| {
            Error::Io(std::io::Error::new(std::io::ErrorKind::NotConnected, "Writer not available"))
        })?;
        ...
        for newline_pos in memchr_iter(b'\n', jsonl_data) {
            if doc_idx < doc_ids.len() && line_start < newline_pos {
                ...
                self.index.insert(doc_ids[doc_idx].clone(), IndexEntry {
                    offset: self.current_offset,
                    length,
                    _padding: 0,
                });
    ...
    pub fn get(&self, doc_id: &str) -> Result<Value> {
        let entry = self.index.get(doc_id)
        ...
        if let Some(mmap) = &self.mmap {
            ...
            if end <= mmap.len() {
                ...
                let mut doc: Value = simd_json::from_slice(&mut buffer)
                    .map_err(|e| Error::Json(serde_json::Error::io(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        e.to_string()
                    ))))?;
                ...
                if let Ok(doc) = serde_json::from_str::<Value>(&line) {
                    if let Some(doc_id) = doc.get("_id").and_then(|v| v.as_str()) {
                        if self.index.contains_key(doc_id) {
                            ...
                            writeln!(writer, "{}", line)?;
                            new_index.insert(doc_id.to_string(), IndexEntry { offset, length, _padding: 0 });
                            offset += length as u64;
                        }
                    }
                }
